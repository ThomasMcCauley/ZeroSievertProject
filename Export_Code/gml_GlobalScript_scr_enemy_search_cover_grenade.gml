var _array;
function scr_enemy_search_cover_grenade(argument0) //gml_Script_scr_enemy_search_cover_grenade
{
                ...
                if (pos_found < pos_found_max)
                {
                    if (ds_grid_get(_grid_check, xx, yy) == false)
                    {
                        ...
                        if (ds_grid_get(obj_controller.grid_motion, _sx, _sy) == 0)
                        {
                            var _new_x = _sx * 16 + 8
                            var _new_y = _sy * 16 + 8
                            if (point_distance(x, y, _new_x, _new_y) > (argument0.radius_min_damage + 16) || (!(instance_line_of_sight(_new_x, _new_y, argument0))))
                            {
                                pos_found++
