function db_cloud_try_restore(argument0, argument1) //gml_Script_db_cloud_try_restore
{
    ...
    if db_cloud_enabled()
    {
        ...
        if steam_file_exists(argument0)
        {
                ...
                switch argument1
                {
                    case 0:
                        if (global.database_was_previously_using_cloud_saves == undefined)
                        {
                            ...
                            _steam_text = steam_file_read(argument0)
                        ...
                        break
                    case 1:
                        __db_trace("Mode=1: Backing up local file then restoring \"", (global.database_savedata_root + argument0), "\" from cloud")
                        _restore = true
                        break
                    case 2:
                        __db_trace("Mode=2: Sending local file to Steam Cloud")
                        __db_trace("Attempting to push to Steam Cloud")
                        _buffer = buffer_load(global.database_savedata_root + argument0)
                        _local_text = buffer_read(_buffer, buffer_string)
                        buffer_delete(_buffer)
            ...
            if _restore
            {
                if file_exists(global.database_savedata_root + argument0)
                {
                    __db_trace("Backing up \"", (global.database_savedata_root + argument0), "\"")
                    db_create_backup(argument0, "backup/backup_")
                }
                else
                    __db_trace("\"", (global.database_savedata_root + argument0), "\" doesn't exist locally")
                __db_trace("Restoring \"", (global.database_savedata_root + argument0), "\" from Steam Cloud")
                if (_steam_text == undefined)
                    _steam_text = steam_file_read(argument0)
                _buffer = buffer_create(string_byte_length(_steam_text), buffer_fixed, 1)
                buffer_write(_buffer, buffer_text, _steam_text)
                buffer_save(_buffer, (global.database_savedata_root + argument0))
                buffer_delete(_buffer)
            }
        }
        else
            __db_trace("\"", argument0, "\" doesn't exist in cloud, not restoring")
    }
}
