if (arms_holder == undefined && weapon_holder.draw_before_follow)
    weapon_holder.weapon_holder.func_draw()
...
if sprite_exists(_backpack_sprite)
{
    if player_state_is(mp_index, gml_Script_scr_player_state_weapon_look)
        ...
        offset_backpack = image_index > 1
    else
        offset_backpack = image_index > 2
...
if (arms_holder != undefined)
    arms_holder.arms_holder.func_draw()
if global.hud_glance_sysytem
{
    if item_exists(arma_now)
    {
        var _get_type_eff_range = item_ammo_get_eff_range(gml_Script_player_loadout_get_ammo_item(self, weapon_slot_now))
        ...
        for (i = 0; i < array_length(_array_points); i++)
        {
            _array_position = _array_points[i]
            draw_circle(x, y, (_array_position[0] * _scope_mult), true)
        }
    }
}
