function glance_effective_range_recoil(argument0, argument1) //gml_Script_glance_effective_range_recoil
{
    if (argument1 == undefined)
        argument1 = 0
    var _mult_dis_player = 1
    var _array_points = glance_get_eff_range_recoil(item_weapon_get_type_effective_range_recoil(argument0))
    if (_array_points == 1)
        return 1;
    var _scope_mult = 1
    if instance_exists(player_get_local())
    {
        ...
        {
            _scope_mult = item_weapon_get_mult_eff_range_aiming(argument0)
    ...
    for (var i = 0; i < array_length(_array_points); i++)
    {
        ...
        if (i < (array_length(_array_points) - 1))
        {
            ...
            if (argument1 >= (_array_position[0] * _scope_mult) && argument1 < (_array_next_pos[0] * _scope_mult))
            {
                _mult_dis_player = lerp(_array_position[1], _array_next_pos[1], ((argument1 - _array_position[0] * _scope_mult) / (_array_next_pos[0] * _scope_mult - _array_position[0] * _scope_mult)))
        ...
        else
        {
            ...
            if (argument1 > (_last_array_pos[0] * _scope_mult))
            {
