...
System.InvalidOperationException: Stack empty.
   ...
   at UndertaleModLib.Decompiler.Decompiler.DirectFunctionCall.DoTypePropagation(DecompileContext context, AssetIDType suggestedType) in D:\a\UndertaleModTool\UndertaleModTool\UndertaleModLib\Decompiler\Instructions\Decompiler.DirectFunctionCall.cs:line 139
*/
function difficulty_set(argument0, argument1) //gml_Script_difficulty_set
{
    ...
    with (_difficulty_def)
    {
        ...
        {
            if (argument1 != variable_struct_get(global.__difficulty_sandbox_values, argument0))
            {
                gml_Script_uiGetData().difficulty_name = "custom"
                variable_struct_set(global.__difficulty_sandbox_values, argument0, argument1)
            }
        }
        else if (argument1 != db_read_ext("shared", "difficulty", argument0, __default))
        {
            gml_Script_uiGetData().difficulty_name = "custom"
            db_open("shared")
            db_write("difficulty", __name, argument1)
            db_close()
        }
    }
}
