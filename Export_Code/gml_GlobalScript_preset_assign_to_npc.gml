function preset_assign_to_npc(argument0) //gml_Script_preset_assign_to_npc
{
    var _npc_id = -1
    if is_in_raid()
    {
        var _map = obj_map_generator.area
        var _lvl = global.livello_now
        var _k = _lvl / 2
        _k = clamp(_k, 1, 3)
        var _preset = "rookie"
    ...
    if (_npc_id == -1)
        trace_error("Unable to assign the preset ", _preset, " for ", argument0)
    return _npc_id;
}
