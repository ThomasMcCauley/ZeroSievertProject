function quest_check_state() //gml_Script_quest_check_state
{
    for (var i = 0; i < 30; i++)
    {
        ...
        if (_quest_id != "")
        {
            ...
            if (_quest_is_complete == true)
            {
                ...
                if (_struct_save.notifica == false)
                {
                    ...
                    _t = language_get_string("Quest") + ": - " + (variable_struct_get(global.quest_database, _quest_id).name) + " - " + language_get_string("completed!")
                    scr_draw_text_with_box(_t, false)
                }
            }
            else
