function gamedata_build_possible_mod_dictionary() //gml_Script_gamedata_build_possible_mod_dictionary
{
    var _i = 0
    repeat array_length(global.item_data_array)
    {
        ...
        if (item_get_category(_mod_id) == "w_mod")
        {
            if item_mod_get_weapon_all(_mod_id)
                var _weapon_array = global.item_all_weapon_array
            else
            ...
            repeat array_length(_weapon_array)
            {
                ...
                if item_mod_fits_weapon(_mod_id, _weapon_id)
                    _mod_id._mod_id.AddMod(variable_struct_get(global.item_possible_mod_data, _weapon_id))
                _j++
            }
        }
        _i++
