function SnapBufferReadXML(argument0, argument1, argument2) //gml_Script_SnapBufferReadXML
{
    ...
    repeat argument2
    {
        _value = buffer_read(argument0, buffer_u8)
        ...
        if (!_skip_whitespace)
        {
            if _in_tag
            {
                ...
                else if _in_value
                {
                    if _in_string
                    {
                        ...
                        {
                            buffer_poke(argument0, (buffer_tell(argument0) - 1), buffer_u8, 0)
                            buffer_seek(argument0, buffer_seek_start, _string_start)
                    ...
                    {
                        _in_string = true
                        _string_start = buffer_tell(argument0)
                    }
                }
                else
                {
                    ...
                    {
                        case 63:
                            if (buffer_tell(argument0) == (_tag_start + 1))
                                _tag_is_prolog = true
                            break
                        case 33:
                            if (buffer_tell(argument0) == (_tag_start + 1))
                        ...
                        case 47:
                            if (buffer_tell(argument0) == (_tag_start + 1))
                                _tag_terminating = true
                            else if (_tag == undefined && buffer_tell(argument0) > _tag_start)
                            {
                                _tag_terminating = true
                                buffer_poke(argument0, (buffer_tell(argument0) - 1), buffer_u8, 0)
                                buffer_seek(argument0, buffer_seek_start, _tag_start)
                                _tag = buffer_read(argument0, buffer_string)
                                buffer_poke(argument0, (buffer_tell(argument0) - 1), buffer_u8, _value)
                                _stack_top = 
                                {
                                    type: _tag
                                }
                ...
                if ((!_in_string) && _value == 62)
                {
                    ...
                    var _previous_value = buffer_peek(argument0, (buffer_tell(argument0) - 2), buffer_u8)
                    ...
                    if ((!_tag_is_comment) && ((!_tag_self_terminating) || _tag_reading_attributes))
                    {
                        ...
                        else
                        {
                            ...
                            _text_start = buffer_tell(argument0)
                        }
                    }
            ...
            else if (_value == 60)
            {
                if _in_text
                {
                    _in_text = false
                    buffer_poke(argument0, (buffer_tell(argument0) - 1), buffer_u8, 0)
                    buffer_seek(argument0, buffer_seek_start, _text_start)
                    _text = buffer_read(argument0, buffer_string)
                ...
                _tag_start = buffer_tell(argument0)
            }
        }
    }
