...
System.InvalidOperationException: Stack empty.
   ...
   at UndertaleModLib.Decompiler.Decompiler.DirectFunctionCall.DoTypePropagation(DecompileContext context, AssetIDType suggestedType) in D:\a\UndertaleModTool\UndertaleModTool\UndertaleModLib\Decompiler\Instructions\Decompiler.DirectFunctionCall.cs:line 139
*/
function __uiCaptureSet(argument0, argument1, argument2) //gml_Script___uiCaptureSet
{
    with (argument0)
    {
        ...
        if (_oldStruct != argument1)
        {
            ...
            {
                _oldStruct._oldStruct.__Call(__root.__captureButtonName, (6 << 0).((6 << 0)).__CallbackGet(_oldStruct))
                with (__root)
                {
            ...
            if (argument1 != undefined)
            {
                with (__root)
                {
                    __captureRef = gml_Script___uiNullableRefCreate(argument1)
                    __captureButtonName = argument2
                    __captureFocus = false
                    __captureTime = current_time
                    __captureLocalX = __pointerX - 0.5 * (argument1.__worldLeft + argument1.__worldRight)
                    __captureLocalY = __pointerY - 0.5 * (argument1.__worldTop + argument1.__worldBottom)
                    __pointerStartX = __pointerX
                    __pointerStartY = __pointerY
                }
                argument1.argument1.__Call(argument2, (4 << 0).((4 << 0)).__CallbackGet(argument1))
            }
        }
    }
}
