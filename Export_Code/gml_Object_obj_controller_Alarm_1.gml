...
for (var j = 0; j < 30; j++)
{
    ...
    if (quest_id_ != "")
    {
        for (var i = 0; i < array_length(variable_struct_get(global.quest_database, quest_id_).objective); i++)
        {
            ...
            {
                case "collect":
                    if is_undefined(variable_struct_get(array_get(global.quest_state, j), "amount_max"))
                    {
...
for (k = 0; k < global.storage_slot_unlocked; k++)
{
    ...
    for (i = 0; i < array_length(_loot_array); i++)
    {
        temp_item_id = _loot_array[i].item
        temp_item_qnt = _loot_array[i].quantity
        for (j = 0; j < 30; j++)
        {
            quest_id_ = global.quest_state[j].id
            if (quest_id_ != "")
...
for (j = 0; j < 30; j++)
{
    ...
    if (quest_id_ != "")
    {
        for (k = 0; k < array_length(variable_struct_get(global.quest_database, quest_id_).objective); k++)
